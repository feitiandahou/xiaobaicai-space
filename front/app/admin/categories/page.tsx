...
export default function AdminCategoriesPage() {
  ...
  useEffect(() => {
    ...
    fetchAdminCategories(token)
      .then((items) => {
        setCategories(items.filter((item) => item.id > 0 && item.name));
      })
      .catch((err) => {
  ...
  const visibleIds = useMemo(() => filteredCategories.map((item) => item.id), [filteredCategories]);
  ...
  const handleStartEdit = (item: CategoryOption) => {
  ...
  const handleSaveEdit = async (id: number) => {
    ...
    setUpdatingId(id);
    try {
      const updated = await updateAdminCategory(token, id, {
        name: finalName,
        slug: finalSlug,
      });
  ...
  const handleBatchDelete = async () => {
    ...
    if (!confirm(`Delete ${targetIds.length} selected categories?`)) return;
    ...
    try {
      ...
      const results = await Promise.allSettled(targetIds.map((id) => deleteAdminCategory(token, id)));
      ...
      if (succeededIds.length > 0) {
        setCategories((current) => current.filter((item) => !succeededIds.includes(item.id)));
  ...
  return (
    <PageTransition className="pt-10 pb-20 max-w-5xl mx-auto">
      ...
      <div className="card-surface rounded-4xl p-6 md:p-8">
        <h1 className="text-4xl leading-none">Categories</h1>
        <p className="text-ink-muted mt-2">Manage content categories used by your posts.</p>
